...
def picking(path, s1, s2, t, find_maxima=True, partition_op=None, multiprocessing_process_num=0, pick_num=None):
    """
    parameters:
    path:file path  s1:sigma1  s2:sigma2  t:threshold level  find_maxima:peaks appears at the maximum/minimum
    multiprocessing_process_num: number of multiporcessing
    partition_op: partition the volume for multithreading, is a dict consists 'nonoverlap_width', 'overlap_width' and
    'save_vg'
    pick_num: the max number of particles to pick out
    # Take a two-dimensional image as an example, if the image size is 210*150(all in pixels), nonoverlap_width is 60
    and overlap_width is 30.
    ...
    In general, s2=1.1*s1, s1 and t depend on particle size and noise. In practice, s1 should be roughly equal to the
    particle radius(in pixels). In related paper, the model achieves highest comprehensive score when s1=7 and t=3.
    ...
    """
    pickle_filename = path[:-4] + f"_peaking_s1_{s1}_{'maxima' if find_maxima else 'minima'}.pickle"
    if not os.path.exists(pickle_filename):
        ...
        # Related paper: Pei L, Xu M, Frazier Z, Alber F. Simulating Cryo-Electron Tomograms of Crowded Mixtures of
        # Macromolecular Complexes and Assessment of Particle Picking. BMC Bioinformatics. 2016; 17: 405.
        ...
        with open(pickle_filename, 'wb') as pickle_file:
            ...
            print(f"write all peaking info to {pickle_filename}")
    else:
...
def main():
    ...
    # filename = 'aitom_demo_single_particle_tomogram.mrc'
    filename = 'INS_21_g3_t26_bin8.rec'
    ...
    # For particular tomogram, larger sigma1 value may have better results. Use IMOD to display selected peaks and
    # determine best sigma1.
    # For 'aitom_demo_cellular_tomogram.mrc', sigma1 is 5 rather than 3 for better performance(in this tomogram,
    # 7nm corresponds to 3.84 pixels)
    ...
    result = picking(path, s1=sigma1, s2=sigma1 * 1.1, t=3, find_maxima=False, partition_op=partition_op,
                     multiprocessing_process_num=10, pick_num=None)
    ...
    # with open(f'{filename[:-4]}_json_file.json', 'w') as f:
    ...
    for i in range(len(result)):
        loc = result[i]['x']
        uuid = result[i]['uuid']
        json_data.append((bytes(filename, "utf-8"), bytes(uuid, "utf-8"), loc))

    # with open(f'{filename[:-4]}.json', 'r') as f:
    #     json_data = json.load(f)

    with open(f'./{filename[:-4]}_info.pickle', 'wb') as pickle_file:
