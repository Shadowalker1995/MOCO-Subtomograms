...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        # 32 x 12 x 12 x 12 -> 16 x 12 x 12 x 12
        out_R = self.conv1(x)
        out_R = self.relu(out_R)
        # 32 x 12 x 12 x 12 -> 16 x 12 x 12 x 12
        out_L = self.conv2(x)
        out_L = self.relu(out_L)
        # 16 x 12 x 12 x 12 -> 32 x 12 x 12 x 12
        out_L = self.conv3(out_L)
        out_L = self.relu(out_L)
        # 32 x 12 x 12 x 12 -> 16 x 12 x 12 x 12
        out_L = self.conv4(out_L)
        # out_L = self.dropout(out_L)
        # (16 x 12 x 12 x 12) + (16 x 12 x 12 x 12) -> 32 x 12 x 12 x 12
...
class RB3D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.maxpool(x)
        # 32 x 12 x 12 x 12 -> 32 x 12 x 12 x 12
        x = self.bottleneck_layers(x)
        # 32 x 12 x 12 x 12 -> 32 x 2 x 2 x 2
        x = self.avgpool(x)
        # 32 x 2 x 2 x 2 -> 256
        x = torch.flatten(x, 1)
        # x = self.dropout(x)
