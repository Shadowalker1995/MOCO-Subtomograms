...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.evaluate:
        stage = 'val'
        stage_normalize = Normalize3D(mean=[0.04725085], std=[13.48426468])
    else:
        stage = 'train'
        stage_normalize = Normalize3D(mean=[0.05964008], std=[13.57436941])
    ...
    if os.path.exists(f'./Figures/{sub_dir}/{stage}_outputs.npy'):
        ...
        model = model_init()
        # Data loading code
        stage_dir = os.path.join(args.data, f'{stage}/subtomogram_mrc')
        stage_json = os.path.join(args.data, f'{stage}/json_label')
        dataset = Custom_CryoET_DataLoader.CryoETDatasetLoader(
            root_dir=stage_dir, json_dir=stage_json,
